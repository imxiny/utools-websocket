 ...
 * 创建Vue实例
 */
var G_ADDRESS = 'ws://127.0.0.1:9501'
        ...
        methods: {
                ...
                try {
                    if (this.connected === false) {
                        ...
                        wsInstance.onerror = function (ev) {
                            // 连接失败的报错
                            if (3 === wsInstance.readyState && false === _this.connected) {
                                _this.writeConsole('danger', `WebSocket connection to '${_this.address}' failed`)
                            }
                            console.error(ev)
                        }
            ...
            sendData: function (raw) {
                ...
                try {
                    ...
                    _this.writeAlert('danger', '消息发送失败: ' + err.toString());
                }
            },
